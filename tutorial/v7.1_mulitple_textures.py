...
shaders_sources = [  # CHANGED!
    ...
    void main()
    {  
        vec4 dif = texture2D(diffuse, out_texture_coordinate);  
        vec4 dec = texture2D(decoration, out_texture_coordinate * 2 - 0.5);
        gl_FragColor = dif + dec;
    }
    """
]
shader_handles = []
...
diffuse_name = create_string_buffer(b'diffuse')
diffuse_location = glGetUniformLocation(program_handle, cast(pointer(diffuse_name), POINTER(c_char)))
decoration_name = create_string_buffer(b'decoration')
decoration_location = glGetUniformLocation(program_handle, cast(pointer(decoration_name), POINTER(c_char)))
...
diffuse_texture = pyglet.image.load('../resources/textures/Container.png').get_texture()  # DIMENSIONS MUST BE POWER OF 2.
...
decoration_texture = pyglet.image.load('../resources/textures/HappyDude.png').get_texture()  # DIMENSIONS MUST BE POWER OF 2.
glBindTexture(GL_TEXTURE_2D, decoration_texture.id)
glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_BASE_LEVEL, 0)
glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAX_LEVEL, 0)
glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR)
glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR)
...
glUniform1i(decoration_location, 1)
